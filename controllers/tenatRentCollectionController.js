...
async function getActiveRentPaymentLink(tenantId) {
  ...
  if (!request?.accessCode) return null;
...
cron.schedule("0 8 * * *", async () => {
  ...
  try {
    ...
    // --------------------------
    // Punishment setting (new JSON-based table)
    // --------------------------
    const punishmentSetting = await PunishmentSetting.findOne();
    ...
    // Inline function to calculate punishment per tenant
    function calculatePunishmentAmount(amount, overdueDays) {
      if (!isPunishmentEnabled || !Array.isArray(rules.rules)) return 0;

      for (const rule of rules.rules) {
        if (
          overdueDays >= rule.from_day &&
          (!rule.to_day || overdueDays <= rule.to_day)
        ) {
    ...
    for (const tenant of tenants) {
      ...
      if (NOTIFY_DAYS.includes(diffDays)) {
        ...
        Unit: Floor ${floor?.floorNumber}, Unit ${unit?.unitNumber}.`;
...
exports.createRentPayment = async (req, res) => {
  try {
    ...
    const totalDays = Math.round(differenceInTime / (1000 * 60 * 60 * 24)) + 1;
...
exports.updateRentPayment = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error updating rent payment", error: error.message });
  }
};
